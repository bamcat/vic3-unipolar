...
VertexStruct VS_OUTPUT_PARTICLE_ADV
{
	float4 Pos				: PDX_POSITION;
	float4 Color			: COLOR;
	...
	float FrameBlend		: TEXCOORD6;
	float2 CustomParameter	: TEXCOORD7;
};
...
VertexShader =
{
	MainCode VertexParticleAdvanced
	{
		Input = "VS_INPUT_PARTICLE"
		Output = "VS_OUTPUT_PARTICLE_ADV"
		Code
		[[
			PDX_MAIN
			{
				VS_OUTPUT_PARTICLE_ADV Out;

				#ifdef REMAP
					float3 InitialOffset = float3( ( Input.UV0 - float2 ( 0.0, 0.0 ) - 0.5f ) * Input.SizeAndOffset.xy, 0 );
					Out.CustomParameter = Input.SizeAndOffset.zw;
				#else
					float3 InitialOffset = float3( ( Input.UV0 - Input.SizeAndOffset.zw - 0.5f ) * Input.SizeAndOffset.xy, 0 );
				#endif
				...
				float3 Direction;
					...
					if( Input.BillboardAxisAndFlipbookTime.x != 0.0 ||
						...
						Input.BillboardAxisAndFlipbookTime.z != 0.0 )
					{
						Up = normalize( RotateVector( RotQ, Input.BillboardAxisAndFlipbookTime.xyz ) );
						Right = normalize( cross( ToCameraDir, Up ) );
					...
					else
					{
						...
						Up = normalize( RotateVector( RotQ, float3( 0.0, 1.0, 0.0 ) ) );
						Right = normalize( cross( RotateVector( RotQ, ToCameraDir ), Up ) );
						Direction = cross( Right, Up );
					}
				#else
					float3 WorldPos = Input.Position.xyz + Offset;
					//Cannot fade steep angles because the lack of a particle normal
					Alpha = Input.Color.a;
					float ViewAngle = dot( ToCameraDir, RotateVector( RotQ, float3( 0.0, 0.0, -1.0 ) ) );

					Up = normalize( RotateVector( RotQ, float3( 0.0, 1.0, 0.0 ) ) );
					Right = normalize( cross( RotateVector( RotQ, float3( 0.0, 0.0, -1.0 ) ), Up ) );
					Direction = cross( Right, Up );
				...
				uint2 FlipbookDimensions = UnpackFromFloat( Input.Position.w );
				float FlipBookTime = Input.BillboardAxisAndFlipbookTime.w % 1.0;
				uint CurrentFrame = CalcCurrentFrame( FlipbookDimensions.x, FlipbookDimensions.y, FlipBookTime );
				uint TotalFrames = FlipbookDimensions.x * FlipbookDimensions.y;
				uint NextFrame = (CurrentFrame + 1) % TotalFrames;

				Out.Pos = FixProjectionAndMul( ViewProjectionMatrix, float4( WorldPos, 1.0f ) );					
				Out.UV0 = CalcCellUV( CurrentFrame, float2( Input.UV0.x, 1.0f - Input.UV0.y ), FlipbookDimensions.x, FlipbookDimensions.y, FlipBookTime );
				Out.UV1 = CalcCellUV( NextFrame, float2( Input.UV0.x, 1.0f - Input.UV0.y ), FlipbookDimensions.x, FlipbookDimensions.y, FlipBookTime );
				Out.FrameBlend = CalcFrameBlend( FlipbookDimensions.x, FlipbookDimensions.y, FlipBookTime );	
				Out.Color = float4( Input.Color.rgb, Alpha );
				Out.WorldSpacePos = WorldPos;
				Out.Normal = Up;
				Out.Tangent = Right;
				Out.Bitangent = Direction;
