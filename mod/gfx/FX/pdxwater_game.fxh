...
PixelShader =
{
	...
	[[
		...
		// Ui Water
		#define UiWaterColor 			float3( 0.005, 0.012, 0.022 )
		#define UiWaterToSun 			float3( -0.5, 0.4, 0.8 )

		// Base normals
		#define UiWaterWaveSpeed 		1.5
		#define UiWaterWave1Scale 		float2( 0.5, 0.5 )
		#define UiWaterWave2Scale 		float2( 10.0, 3.0 )
		#define UiWaterWave3Scale 		float2( 6.0, 6.0 )
		#define UiWaterNormalUvScale 	0.075
		#define UiWaterNormalFlatten 	2.0

		// Flownormal (Main normal used at oceans)
		#define UiWaterFlowNormalUvScale 	0.3
		#define UiWaterFlowNormalBlend 		0.75
		#define UiWaterFlowNormalSpeed 		-0.05
		#define UiWaterFlowNormalFlatten	1.5

		// Specular Highlight
		#define UiWaterGlossScale 		0.9
		#define UiWaterSpecular 		0.05

		// Cubemap
		#define UiWaterWaterFresnelBias 0.01
		#define UiWaterFresnelPow 		4.00

		// Simulated movement
		#define UiWaterBoatSpeed 		2.55 	// Tried to match back particles speed
	]]
...
PixelShader =
{
	...
	Code
	[[
		SWaterMeshShaderData GetWaterMeshShaderData( uint InstanceIndex )
		{
			SWaterMeshShaderData UserData;
			UserData._ShipSpeed = Data[ InstanceIndex + PDXMESH_USER_DATA_OFFSET + 0 ].x;

			return UserData;
		}

		float4 SampleNormalMapTextureAlpha( PdxTextureSampler2D Texture, float2 UV, float2 Scale, float Rotation, float Offset, float NormalFlatten )
		{
			float2 Rotate = float2( cos( Rotation ), sin( Rotation ) );

			float2 UVCoord = UV;
			UVCoord = float2( UVCoord.x * Rotate.x - UVCoord.y * Rotate.y, UVCoord.x * Rotate.y + UVCoord.y * Rotate.x );
			UVCoord *= Scale;
			UVCoord.y += Offset;

			float4 NormalSample =  PdxTex2DUpscaleNative( Texture, UVCoord );
			float3 Normal = UnpackNormal( NormalSample ).xzy;
			...
			return float4( normalize( Normal ), NormalSample.a );
		}
		...
		float3 CalcUiFlowGame( PdxTextureSampler2D FlowMapTexture, PdxTextureSampler2D FlowNormalTexture, out float FoamMask, float3 WorldSpacePos, float ShipSpeed )
		{
			WorldSpacePos.z -= GlobalTime * UiWaterBoatSpeed * ShipSpeed;

			float FlowMapScale = 10.0;
			float2 FlowMapUv = WorldSpacePos.xz * 0.25;
			float2 FlowCoordScale = _WaterFlowMapSize * FlowMapScale;
			float2 FlowCoord = FlowMapUv * FlowCoordScale;

			float2 BlendFactor = abs( 2.0 * frac( FlowMapUv ) - 1.0 ) - 0.5;
			BlendFactor = 0.5 - 4.0 * BlendFactor * BlendFactor * BlendFactor;

			float2 NormalCoord = WorldSpacePos.xz * UiWaterFlowNormalUvScale;
			float2 DDX = ddx( NormalCoord );
			float2 DDY = ddy( NormalCoord );
			...
			float4 Sample1;
			SampleFlowTexture( FlowMapTexture, FlowNormalTexture, floor( FlowMapUv ) / FlowCoordScale, NormalCoord, Offset, DDX, DDY, Sample1.xyz, Sample1.a );
			float4 Sample2;
			SampleFlowTexture( FlowMapTexture, FlowNormalTexture, floor( FlowMapUv + float2( 0.5, 0.0 ) ) / FlowCoordScale, NormalCoord, Offset, DDX, DDY, Sample2.xyz, Sample2.a );
			float4 Sample3;
			SampleFlowTexture( FlowMapTexture, FlowNormalTexture, floor( FlowMapUv + float2( 0.0, 0.5 ) ) / FlowCoordScale, NormalCoord, Offset, DDX, DDY, Sample3.xyz, Sample3.a );
			float4 Sample4;
			SampleFlowTexture( FlowMapTexture, FlowNormalTexture, floor( FlowMapUv + float2( 0.5, 0.5 ) ) / FlowCoordScale, NormalCoord, Offset, DDX, DDY, Sample4.xyz, Sample4.a );
			...
			Sample.y *= UiWaterFlowNormalFlatten;
			float3 Normal = normalize( Sample.xyz );
		...
		float4 GameCalcWater( in SWaterParameters Input )
		{
			...
			FoamFactor += GameCalcFoamFactorWaves( Input._WorldUV, Input._WorldSpacePos.xz, Input._Depth, Input._FlowFoamMask, Input._FlowNormal );
		...
		float4 GameCalcWaterUi( in SWaterParameters Input, uint InstanceIndex )
		{

			SWaterMeshShaderData UserData;
			UserData = GetWaterMeshShaderData( InstanceIndex );
			...
			// Normals
			Input._WorldSpacePos.z -= GlobalTime * UiWaterBoatSpeed * UserData._ShipSpeed;
			float2 UVCoord = Input._WorldSpacePos.xz * float2( 1.0, -1.0 ) * UiWaterNormalUvScale;
			float3 NormalMap1 = SampleNormalMapTexture( AmbientNormalTexture, UVCoord, UiWaterWave1Scale, _WaterWave1Rotation, JOMINIWATER_GlobalTime * _WaterWave1Speed * UiWaterWaveSpeed, UiWaterNormalFlatten );
			float3 NormalMap2 = SampleNormalMapTexture( AmbientNormalTexture, UVCoord, UiWaterWave2Scale, _WaterWave2Rotation, JOMINIWATER_GlobalTime * _WaterWave2Speed * UiWaterWaveSpeed, UiWaterNormalFlatten );
			float3 NormalMap3 = SampleNormalMapTexture( AmbientNormalTexture, UVCoord, UiWaterWave3Scale, _WaterWave3Rotation, JOMINIWATER_GlobalTime * _WaterWave3Speed * UiWaterWaveSpeed, UiWaterNormalFlatten );
			float3 Normal = NormalMap1 + NormalMap2 + NormalMap3;
			Normal = normalize( Normal + Input._FlowNormal );

			float FoamFactor = GameCalcOceanWaves( UVCoord, Input._WorldSpacePos.xz * UiWaterNormalUvScale * 5.0, Input._FlowFoamMask, Input._FlowNormal, 0.8 );
			...
			lightingProperties._Normal = Normal;
			lightingProperties._Diffuse = WaterDiffuse + FoamFactor;
			lightingProperties._Glossiness = UiWaterGlossScale;
			lightingProperties._SpecularColor = vec3( UiWaterSpecular );
			lightingProperties._NonLinearGlossiness = GetNonLinearGlossiness( lightingProperties._Glossiness );

			// Ocean specular
			float3 DiffuseLight = vec3( 0.0f );
			float3 SpecularLight = vec3( 0.0f );
			float3 ToSun = normalize( UiWaterToSun );
			CalculateSunLight( lightingProperties, 1.0f, ToSun, DiffuseLight, SpecularLight );
			float3 FinalColor = ComposeLight( lightingProperties, 1.0f, ToSun, DiffuseLight, SpecularLight * _WaterSpecularFactor );

			// Cubemap reflection
			float3 Reflection = CalcReflection( Normal, ToCameraDir );
			float FresnelFactor = Fresnel( abs( dot( lightingProperties._ToCameraDir, Normal ) ), UiWaterWaterFresnelBias, UiWaterFresnelPow );
			FinalColor += lerp( UiWaterColor, Reflection, FresnelFactor );
