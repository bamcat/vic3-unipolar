	...
	MainCode VertexShader
	{
		...
		[[
			PDX_MAIN
			{
				...
				// Clamp to water level so borders over submerged terrain stay above the water plane.
				float TerrainHeight = max( GetHeight( Input.Center ), _WaterHeight );
				float3 Position = float3( Input.Position.x, TerrainHeight, Input.Position.y );
